...
let editingId = null;
let editingPaymentId = null; // New tracking for payment edits
...
function renderNotebookList() {
    ...
    if(!currentFile || currentFile.entries.length === 0) {
        ...
        currentFile.entries.forEach(entry => {
            sumNet += parseFloat(entry.netAmount) || 0;
            // Now outputting DD-MM-YYYY formatting
            listHTML += `
            <tr style="border-bottom: 1px solid #ccc; color: black; white-space: nowrap;">
                <td style="padding: 5px; border: 1px solid #ccc;">${formatDate(entry.date)}</td>
                <td style="padding: 5px; border: 1px solid #ccc;">${entry.vehicle}</td>
                <td style="padding: 5px; border: 1px solid #ccc;">${entry.mc1}</td>
                <td style="padding: 5px; border: 1px solid #ccc;">${entry.netWT}</td>
                <td style="padding: 5px; border: 1px solid #ccc;">${entry.netWeight}</td>
...
function addPayment() {
    ...
    let payData = {
        id: editingPaymentId ? editingPaymentId : Date.now(),
        date: document.getElementById('payDate').value || "",
        name: document.getElementById('payName').value || "",
        amount: document.getElementById('payAmount').value || "0"
    };
    ...
    if (editingPaymentId) {
        currentFile.payments = currentFile.payments.map(p => p.id === editingPaymentId ? payData : p);
        editingPaymentId = null;
        let saveBtn = document.getElementById('paySaveBtn');
        if(saveBtn) saveBtn.innerText = "➕ Add";
        if(typeof showCustomAlert === "function") showCustomAlert("Payment Updated!");
    } else {
        currentFile.payments.push(payData);
        if(typeof showCustomAlert === "function") showCustomAlert("Payment Added!");
    }
...
function editPayment(id) {
    let currentFile = getCurrentFile();
    let pay = currentFile.payments.find(p => p.id === id);
    if (pay) {
        document.getElementById('payDate').value = pay.date || "";
        document.getElementById('payName').value = pay.name || "";
        document.getElementById('payAmount').value = pay.amount || "";
        editingPaymentId = id;
        
        let saveBtn = document.getElementById('paySaveBtn');
        if(saveBtn) saveBtn.innerText = "✏️ Update";
        window.scrollTo({ top: document.getElementById('payName').offsetTop - 50, behavior: 'smooth' });
    }
}
...
function renderPaymentList() {
    ...
    if(!currentFile || !currentFile.payments || currentFile.payments.length === 0) {
        ...
        currentFile.payments.forEach(pay => {
            ...
            listHTML += `
            <tr style="border-bottom: 1px solid #ccc; color: black; white-space: nowrap;">
                <td style="padding: 5px; border: 1px solid #ccc;">${formatDate(pay.date)}</td>
                <td style="padding: 5px; border: 1px solid #ccc;">${pay.name}</td>
                <td style="padding: 5px; border: 1px solid #ccc; font-weight:bold;">${pay.amount}</td>
                <td data-html2canvas-ignore="true" style="padding: 5px; border: 1px solid #ccc; text-align: center;">
                    <button onclick="editPayment(${pay.id})" style="margin:2px; padding: 4px 8px; font-size: 12px; background: #3B82F6;">✏️</button>
                    <button onclick="deletePayment(${pay.id})" style="margin:2px; padding: 4px 8px; font-size: 12px; background: #EF4444;">❌</button>
                </td>
            </tr>`;
        });
...
function shareToWhatsApp() {
    ...
    let sumNet = 0; let totalPaid = 0;
    currentFile.entries.forEach(e => sumNet += parseFloat(e.netAmount) || 0);
    if(currentFile.payments) currentFile.payments.forEach(p => totalPaid += parseFloat(p.amount) || 0);
    
    let balance = sumNet - totalPaid;
    
    let waText = `*Ledger Update: ${currentFile.name}*\n\n`;
    waText += `📊 *Total Net Amount:* ₹${sumNet}\n`;
    waText += `💸 *Total Paid:* ₹${totalPaid}\n`;
    waText += `✅ *Remaining Balance:* ₹${balance}\n\n`;
    waText += `_Generated securely via Harvest Book_`;
    
    let url = `https://wa.me/?text=${encodeURIComponent(waText)}`;
...
function exportToExcel() {
    let currentFile = getCurrentFile();
    if (!currentFile || currentFile.entries.length === 0) return;
    let sumNetAmount = 0; let sumPayAmount = 0;

    if (currentFile.payments && currentFile.payments.length > 0) {
        currentFile.payments.forEach(pay => { sumPayAmount += parseFloat(pay.amount) || 0; });
    }
    sumPayAmount = Math.round(sumPayAmount);
    
    let csv = "DATE,VEHICLE,MC,NET WT,NET WEIGHT,MC,MC CUT,FINAL WEIGHT,RATE,AMOUNT,FRIGHTDIF,FRIGHT,RUSUM,NET AMOUNT\n";
    currentFile.entries.forEach(e => {
        csv += `"${formatDate(e.date)}","${e.vehicle}","${e.mc1}","${e.netWT}","${e.netWeight}","${e.mc2}","${e.mcCut}","${e.finalWeight}","${e.rate}","${e.amount}","${e.frightDif}","${e.fright}","${e.rusum}","${e.netAmount}"\n`;
        sumNetAmount += parseFloat(e.netAmount) || 0;
    });
    ...
    if (currentFile.payments && currentFile.payments.length > 0) {
        currentFile.payments.forEach(pay => {
            csv += ["", "", "", "", formatDate(pay.date), "", "", pay.name, "", "", "", "", "", pay.amount].map(val => val ? `"${val}"` : "").join(",") + "\n";
        });
        csv += ["", "", "", "", "", "", "", "TOTAL PAID:", "", "", "", "", "", sumPayAmount].map(val => val !== "" ? `"${val}"` : "").join(",") + "\n";
    }
    
    let link = document.createElement("a");
    link.download = `${currentFile.name || "Notebook"}.csv`;
    link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }));
    link.click();
}
...
function exportToImage() {
    ...
    // 2. Force container to full width so nothing is hidden
...
function exportToPDF() {
    ...
    html2canvas(container, { 
        ...
        // Changed to 'l' for Landscape orientation to fit wide columns
        let pdf = new window.jspdf.jsPDF('l', 'mm', 'a4'); 
        let pdfWidth = pdf.internal.pageSize.getWidth();
        let pdfHeight = (canvas.height * pdfWidth) / canvas.width;
        
        // Custom height prevents cutting off the bottom rows
        let customPdf = new window.jspdf.jsPDF('l', 'mm', [pdfWidth, pdfHeight + 20]);
        let name = getCurrentFile().name || "Notebook";
        
        customPdf.text(name, 14, 15);
        customPdf.addImage(imgData, 'PNG', 0, 20, pdfWidth, pdfHeight);
        customPdf.save(`${name}.pdf`);
    });
}