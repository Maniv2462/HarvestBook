...
function loadFromHistory(id) {
    ...
    if(note) {
        ...
        if (typeof calcFinal === "function") calcFinal();
        
        toggleHistory();
        window.scrollTo({ top: 0, behavior: 'smooth' });
        
        // NO MORE BROWSER ALERT! Uses the beautiful green popup now.
        if (typeof showCustomAlert === "function") {
            showCustomAlert("✅ Note Loaded Successfully!");
        }
    }
}

// Deletes a specific note from history
function deleteFromHistory(id) {
    // NO MORE BROWSER CONFIRM! Uses the nice popup modal now.
    if (typeof showConfirm === "function") {
        showConfirm("Are you sure you want to delete this note permanently?", () => {
            let history = JSON.parse(localStorage.getItem('if_history')) || [];
